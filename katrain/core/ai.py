...
class ScoreLossStrategy(AIStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if top_cand.is_pass:
            self.game.katrain.log(
                f"[ScoreLossStrategy] Top move is pass, so passing regardless of strategy", OUTPUT_DEBUG
            )
...
class OwnershipBaseStrategy(AIStrategy):
    ...
    def get_moves_with_settledness(self):
        ...
        # Sort moves by score
        sorted_moves = sorted(moves_data, key=lambda t: t[6])  # Sort by the precalculated score
...
class SimpleOwnershipStrategy(OwnershipBaseStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if top_cand.is_pass:
            self.game.katrain.log(
                f"[SimpleOwnershipStrategy] Top move is pass, so passing regardless of strategy", OUTPUT_DEBUG
            )
            return top_cand, "Top move is pass, so passing regardless of strategy."
...
class SettleStonesStrategy(OwnershipBaseStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if top_cand.is_pass:
            self.game.katrain.log(
                f"[SettleStonesStrategy] Top move is pass, so passing regardless of strategy", OUTPUT_DEBUG
            )
            return top_cand, "Top move is pass, so passing regardless of strategy."
...
class PolicyStrategy(AIStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if not self.cn.policy:
            self.game.katrain.log(
                f"[PolicyStrategy] No policy data available, falling back to DefaultStrategy", OUTPUT_DEBUG
            )
            return DefaultStrategy(self.game, self.settings).generate_move()
...
class WeightedStrategy(AIStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if not self.cn.policy:
            self.game.katrain.log(
                f"[WeightedStrategy] No policy data available, falling back to DefaultStrategy", OUTPUT_DEBUG
            )
            return DefaultStrategy(self.game, self.settings).generate_move()
...
class PickStrategy(PickBasedStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        self.game.katrain.log(
            f"[PickStrategy] Starting move generation using base PickBasedStrategy implementation", OUTPUT_DEBUG
        )
...
class LocalStrategy(PickBasedStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if not (self.cn.move and self.cn.move.coords):
            self.game.katrain.log(
                f"[LocalStrategy] No previous move with valid coordinates found, falling back to WeightedStrategy",
                OUTPUT_DEBUG,
            )
            self.game.katrain.log(
                f"[LocalStrategy] Using default weighted settings: pick_override=0.9, weaken_fac=1, lower_bound=0.02",
                OUTPUT_DEBUG,
            )
...
class TenukiStrategy(PickBasedStrategy):
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        if not (self.cn.move and self.cn.move.coords):
            self.game.katrain.log(
                f"[TenukiStrategy] No previous move with valid coordinates found, falling back to WeightedStrategy",
                OUTPUT_DEBUG,
            )
            self.game.katrain.log(
                f"[TenukiStrategy] Using default weighted settings: pick_override=0.9, weaken_fac=1, lower_bound=0.02",
                OUTPUT_DEBUG,
            )
            return WeightedStrategy(
                self.game, {"pick_override": 0.9, "weaken_fac": 1, "lower_bound": 0.02}
    ...
    def generate_weighted_coords(self, legal_policy_moves, policy_grid, size):
        ...
        self.game.katrain.log(
            f"[TenukiStrategy] Generating tenuki-based weights (far from previous move)", OUTPUT_DEBUG
        )
    ...
    def generate_move(self) -> Tuple[Move, str]:
        ...
        def set_analysis(a, partial_result):
            ...
            if not partial_result:
                ...
                self.game.katrain.log(
                    f"[HumanStyleStrategy] Received partial analysis results - ignoring", OUTPUT_DEBUG
                )
        ...
        if error or not analysis:
            ...
            if policy_move:
                ...
                self.game.katrain.log(
                    f"[HumanStyleStrategy] No policy moves available for fallback - will return pass", OUTPUT_DEBUG
                )
